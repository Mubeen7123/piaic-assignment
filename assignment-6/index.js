...
let positivearry = mixarry.filter((element) => {
    if (element >= 0) {
        return element;
    }
});
...
let tempratureCelcious = [0, 10, 20, 30, 40];
let fohrenheit = tempratureCelcious.map((celcious) => {
...
let fruitsabove5Character = fruits.filter((elements) => {
